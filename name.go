...
func StringToName(s string) (val uint64, err error) {
	// ported from the roxe codebase, libraries/chain/include/roxe/chain/name.hpp
	var i uint32
	sLen := uint32(len(s))
	for ; i <= 12; i++ {
		var c uint64
...
var roxeNameUint64 = uint64(6138663577826885632)
var roxeTokenNameUint64 = uint64(6138663591592764928)
var cachedNames = map[uint64]string{
	6138663577826885632: "roxe",
	6138663591592764928: "roxe.token",
}
...
func NameToString(in uint64) string {
	...
	// ported from libraries/chain/name.cpp in roxe
	a := []byte{'.', '.', '.', '.', '.', '.', '.', '.', '.', '.', '.', '.', '.'}
