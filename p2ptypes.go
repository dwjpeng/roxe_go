...
//        working for either version, so supporting ROXE 2.0.x only is a fair improvements. Will need
...
type BlockState struct {
	...
	ActivatedProtocolFeatures *ProtocolFeatureActivationSet `json:"activated_protocol_features,omitempty" roxe:"optional"`
	AdditionalSignatures      []ecc.Signature               `json:"additional_signatures"`

	SignedBlock *SignedBlock `json:"block,omitempty" roxe:"optional"`
	Validated   bool         `json:"validated"`

	// ROXE 1.x
	BlockSigningKeyV1 *ecc.PublicKey `json:"block_signing_key,omitempty" roxe:"-"`
}
...
func (p *ProducerScheduleOrAuthoritySchedule) UnmarshalJSON(data []byte) error {
	...
	// what happens on marshal? Both are defined, that's what we choose for now, `roxe-go` user
...
func (p *ProducerScheduleOrAuthoritySchedule) UnmarshalBinary(decoder *Decoder) error {
	...
	//        `nodroxe` at the binary level. In the block header, there is some header
...
type BlockHeader struct {
	...
	// ROXE 1.x
	NewProducersV1 *ProducerSchedule `json:"new_producers,omitempty" roxe:"optional"`
