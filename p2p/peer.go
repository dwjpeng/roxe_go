...
type HandshakeInfo struct {
	...
	HeadBlockNum             uint32
	HeadBlockID              roxe.Checksum256
	HeadBlockTime            time.Time
	LastIrreversibleBlockNum uint32
	LastIrreversibleBlockID  roxe.Checksum256
}
...
func (p *Peer) Read() (*roxe.Packet, error) {
	packet, err := roxe.ReadPacket(p.reader)
...
func (p *Peer) WriteP2PMessage(message roxe.P2PMessage) (err error) {
...
func (p *Peer) SendSyncRequest(startBlockNum uint32, endBlockNumber uint32) (err error) {
	...
	syncRequest := &roxe.SyncRequestMessage{
...
func (p *Peer) SendRequest(startBlockNum uint32, endBlockNumber uint32) (err error) {
	...
	request := &roxe.RequestMessage{
		ReqTrx: roxe.OrderedBlockIDs{
			Mode:    [4]byte{0, 0, 0, 0},
			Pending: startBlockNum,
		},
		ReqBlocks: roxe.OrderedBlockIDs{
			Mode:    [4]byte{0, 0, 0, 0},
...
func (p *Peer) SendNotice(headBlockNum uint32, libNum uint32, mode byte) error {
	...
	notice := &roxe.NoticeMessage{
		KnownTrx: roxe.OrderedBlockIDs{
			Mode:    [4]byte{mode, 0, 0, 0},
			Pending: headBlockNum,
		},
		KnownBlocks: roxe.OrderedBlockIDs{
			Mode:    [4]byte{mode, 0, 0, 0},
			Pending: libNum,
		},
	}
...
func (p *Peer) SendHandshake(info *HandshakeInfo) error {
	...
	tstamp := roxe.Tstamp{Time: info.HeadBlockTime}
	...
	handshake := &roxe.HandshakeMessage{
		NetworkVersion:           1206,
