...
// that can unmarshal an ROXE binary description of themselves.
//
...
func (d *Decoder) decodeStruct(v interface{}, t reflect.Type, rv reflect.Value) (err error) {
	...
	for i := 0; i < l; i++ {
		...
		tag := structField.Tag.Get("roxe")
		if tag == "-" {
			continue
		}

		if tag != "binary_extension" && seenBinaryExtensionField {
			panic(fmt.Sprintf("the `roxe: \"binary_extension\"` tags must be packed together at the end of struct fields, problematic field %s", structField.Name))
		}
