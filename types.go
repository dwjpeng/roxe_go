...
// For reference:
// https://github.com/mithrilcoin-io/EosCommander/blob/master/app/src/main/java/io/mithrilcoin/roxecommander/data/remote/model/types/EosByteWriter.java
...
// ROXESymbol represents the standard ROXE symbol on the chain.  It's
...
// TNTSymbol represents the standard ROXE Testnet symbol on the testnet chain.
// Temporary Network Token (TNT) is the native token of the ROXE Testnet.
// It's here just to speed up things.
var TNTSymbol = Symbol{Precision: 4, Symbol: "TNT"}

func NewROXEAsset(amount int64) Asset {
	return Asset{Amount: Int64(amount), Symbol: ROXESymbol}
}

// NewAsset reads from a string an ROXE asset.
//
...
// NewAssetFromString reads a string an decode it to an roxe.Asset
...
func NewROXEAssetFromString(input string) (Asset, error) {
	return NewFixedSymbolAssetFromString(ROXESymbol, input)
}
...
// https://github.com/ROXE/fc/blob/0e74738e938c2fe0f36c5238dbc549665ddaef82/include/fc/variant.hpp#L47
type Blob string
...
// It's the ordering that defines the binary variant value just like in native `nodroxe` C++
...
func (a *BaseVariant) UnmarshalJSON(data []byte, def *VariantDefinition) error {
	...
	if typeGo.Kind() == reflect.Ptr {
		...
		// the equivalence of doing `name := roxe.Name("")`) and take the
...
//
// **Note** For `Int64` and `Uint64`, we return `roxe.Int64` and `roxe.Uint64` types
