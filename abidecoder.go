...
// Decodes the ROXE ABIs built-in types
func (a *ABI) read(binaryDecoder *Decoder, fieldType string) (interface{}, error) {
	...
	case "float64":
		...
		if e == nil {
			value = formatFloat(v, a.fitNodroxe)
		}
		err = e
	case "float128":
	...
	case "time_point":
		...
		if e == nil {
			value = formatTimePoint(timePoint, a.fitNodroxe)
		}
		err = e
	case "time_point_sec":
...
const nodroxeTimePointFormat = "2006-01-02T15:04:05.000"

func formatTimePoint(timePoint TimePoint, shouldFitNodroxe bool) string {
	t := time.Unix(0, int64(timePoint*1000))
	if shouldFitNodroxe {
		return t.UTC().Format(nodroxeTimePointFormat)
	}
...
func formatFloat(v float64, fitNodroxe bool) interface{} {
	switch {
	case math.IsInf(v, 1):
		return "inf"
	case math.IsInf(v, -1):
