...
func TestActionNewAccount(t *testing.T) {
	...
	a := &roxe.Action{
		Account: roxe.AccountName("roxe"),
		Name:    roxe.ActionName("newaccount"),
		Authorization: []roxe.PermissionLevel{
			{
				Actor:      roxe.AccountName("roxe"),
				Permission: roxe.PermissionName("active"),
			},
		},
		ActionData: roxe.NewActionData(NewAccount{
			Creator: roxe.AccountName("roxe"),
			Name:    roxe.AccountName("abourget"),
	...
	tx := &roxe.Transaction{
		Actions: []*roxe.Action{a},
	}

	buf, err := roxe.MarshalBinary(tx)
	// println(string(buf))
	...
	assert.NoError(t, err)
	assert.Equal(t, `{"account":"roxe","name":"newaccount","authorization":[{"actor":"roxe","permission":"active"}],"data":"0000000000ea305500000059b1abe93101000000010002c0ded2bc1f1305fb0faac5e6c03ee3a1924234985427b6167ca569d13df435cf0100000001000000010002c0ded2bc1f1305fb0faac5e6c03ee3a1924234985427b6167ca569d13df435cf01000000"}`, string(buf))

	buf, err = json.Marshal(a.ActionData.Data)
	assert.NoError(t, err)

	assert.Equal(t, "{\"creator\":\"roxe\",\"name\":\"abourget\",\"owner\":{\"threshold\":1,\"keys\":[{\"key\":\""+ecc.PublicKeyPrefixCompat+"6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"weight\":1}]},\"active\":{\"threshold\":1,\"keys\":[{\"key\":\""+ecc.PublicKeyPrefixCompat+"6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"weight\":1}]}}", string(buf))
...
func TestMarshalTransactionAndSigned(t *testing.T) {
	...
	tx := &roxe.SignedTransaction{Transaction: &roxe.Transaction{
		Actions: []*roxe.Action{a},
	}}
	...
	buf, err = json.Marshal(a)
	assert.NoError(t, err)
	assert.Equal(t, `{"account":"roxe","name":"newaccount","authorization":[{"actor":"roxe","permission":"active"}],"data":"0000000000ea305500000059b1abe9310000000000000000000000000000"}`, string(buf))
}
...
func TestMarshalTransactionAndPack(t *testing.T) {
	...
	b := &roxe.Action{
		...
		ActionData: roxe.NewActionData(NewAccount{
			Creator: roxe.AccountName("roxe"),
			Name:    roxe.AccountName("cbillett"),
		}),
	}

	tx := &roxe.Transaction{
		Actions: []*roxe.Action{a, b},
	}

	buf, err := json.Marshal(tx)
	fmt.Println("Transaction: ", string(buf))

	signedTx := &roxe.SignedTransaction{Transaction: tx}
	buf, err = json.Marshal(signedTx)
	fmt.Println("Signed Transaction: ", string(buf))

	packedTx, err := signedTx.Pack(roxe.CompressionNone)
	assert.NoError(t, err)
