...
import (
	...
	"github.com/dwjpeng/roxe_go/token"
)

func ExampleAPI_PushTransaction_transfer_ROXE() {
	...
	from := roxe.AccountName("roxeuser1")
	to := roxe.AccountName("roxeuser2")
	quantity, err := roxe.NewROXEAssetFromString("1.0000 ROC")
	memo := ""
	if err != nil {
		panic(fmt.Errorf("invalid quantity: %w", err))
	}

	txOpts := &roxe.TxOptions{}
	if err := txOpts.FillFromChain(context.Background(), api); err != nil {
		panic(fmt.Errorf("filling tx opts: %w", err))
	}

	tx := roxe.NewTransaction([]*roxe.Action{token.NewTransfer(from, to, quantity, memo)}, txOpts)
	signedTx, packedTx, err := api.SignTransaction(context.Background(), tx, txOpts.ChainID, roxe.CompressionNone)
	if err != nil {
