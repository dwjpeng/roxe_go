...
func TestDecoder_BlockState(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			expected := strings.ReplaceAll(string(expectedFileContent), `"ROXE`, `"`+ecc.PublicKeyPrefixCompat)
...
func TestDecoder_Encode(t *testing.T) {
	...
	assert.Equal(t, "ROXE", s.F18.Symbol.Symbol)
...
func TestDecoder_Decode_struct_tag_BinaryExtension(t *testing.T) {
	type BinaryExtensionTestStruct struct {
		S2 string
		S1 string `roxe:"binary_extension"`
	}
...
func TestDecoder_Decode_struct_tag_BinaryExtension_NotGrouped(t *testing.T) {
	type BinaryExtensionTestStruct struct {
		S1 string
		S2 string `roxe:"binary_extension"`
		S3 string
	}

	require.PanicsWithValue(t, "the `roxe: \"binary_extension\"` tags must be packed together at the end of struct fields, problematic field S3", func() {
		var s BinaryExtensionTestStruct
...
func TestDecoder_SignedBlock_Full(t *testing.T) {
	...
	//       nodroxe binary data, which is usually all big endian, it is written as
	...
	//       need to craft a block with some data in it or search `nodroxe` to validate how a `vector<char>`
	...
	assert.Equal(t, AccountName("roxe"), signedBlock.Producer)
